...
//
//}
void draw_player (CONST int x, CONST int y, CONST float r, CONST int z, CONST int r1)
{
  int             x1, y1;
  PutBitmap ((int) x - ROCKET_RADIUS / DIV, (int) y - ROCKET_RADIUS / DIV,
    rocket_bitmap[z], physicalscreen);
  x1 = x * DIV + sin (r - RAD (30)) * r1 - EYE_RADIUS;
  y1 = y * DIV + cos (r - RAD (30)) * r1 - EYE_RADIUS;
  PutBitmap (x1 / DIV, y1 / DIV,
    eye_bitmap[z], physicalscreen);
  x1 = x * DIV + sin (r + RAD (30)) * r1 - EYE_RADIUS;
  y1 = y * DIV + cos (r + RAD (30)) * r1 - EYE_RADIUS;
  PutBitmap (x1 / DIV, y1 / DIV,
    eye_bitmap[z], physicalscreen);
...
//	  SetScreen (backscreen);
    CopyVSToVS(starbackground, physicalscreen);
	  EnableClipping ();
	  for (z = 0; z < 3; z++)
	    {
