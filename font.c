...
//DKS - removed excess non-ARM/MIPS stuff
...
//DKS - main part we'll re-write for SDL
//static INLINE int
//gl_regioncode (CONST int x, CONST int y)
//{
//  int             result = 0;
...
//	while (x != x2)
//	  {
//	    recentpoint = *point = colors_16bit[c];
//	    if (y > 3 * MAPHEIGHT / 5)
//	      {
//		*(point + 1) = colors_16bit[c];
//		if (y > 3 * MAPHEIGHT / 4)
//		  *(point + 2) = colors_16bit[c];
//	      }
//	    if (d > 0 || (d == 0 && sx == 1))
//	      {
//		y += sy;
...
//DKS #define gl_putpixel(x,y,c) FillRectangle(x,y,y>3 * MAPHEIGHT / 4 && DIV == 1?2:1,y>3 * MAPHEIGHT / 5 && DIV == 1?2:1,c)
...
//DKS - changed (again) this now that line drawing is done by us, not through sdl_gfx
//#define vga_drawline(x1,y1,x2,y2) \
//  lineColor(physicalscreen, ((int)(MAPWIDTH/2+(x1)*220/(1000-(y1))/DIV)), \
//          ((int)(MAPHEIGHT/3+MAPWIDTH*220/(1000-(y1)))), \
//          ((int)(MAPWIDTH/2+(x2)*220/(1000-(y2))/DIV)), \
//          ((int) (MAPHEIGHT/3+MAPWIDTH*220/(1000-(y2)))), \
//          colors_RGBA[textcolor])
#define vga_drawline(x1,y1,x2,y2) \
  SDL_DrawLine(physicalscreen,((int)(MAPWIDTH/2+(x1)*220/(1000-(y1))/DIV)), \
          ((int)(MAPHEIGHT/3+MAPWIDTH*220/(1000-(y1)))), \
          ((int)(MAPWIDTH/2+(x2)*220/(1000-(y2))/DIV)), \
          ((int) (MAPHEIGHT/3+MAPWIDTH*220/(1000-(y2)))), \
          colors_RGBA[textcolor])
...
//	        textcolor)
//#define vga_drawhline(x1,y1,x2,y2)\
//  hlineColor(physicalscreen, (int)(MAPWIDTH/2+(x1)*220/(1000-(y1))/DIV), \
//            ((int)(MAPWIDTH/2+(x2)*220/(1000-(y2))/DIV)),\
//            ((int)(MAPHEIGHT/3+MAPWIDTH*220/(1000-(y1)))),\
//            colors_RGBA[textcolor])
#define vga_drawhline(x1,y1,x2,y2)\
	SDL_DrawHLine(physicalscreen,(int)(MAPWIDTH/2+(x1)*220/(1000-(y1))/DIV),(int)(MAPHEIGHT/3+MAPWIDTH*220/(1000-(y1))),\
	        (int)(MAPWIDTH/2+(x2)*220/(1000-(y2))/DIV),\
	        colors_RGBA[textcolor])
...
//DKS - gonna try to re-write this one more time to fix possible clipping problems..
//static INLINE void vga_drawpixel(x1, y1) {
//  if SDL_MUSTLOCK(physicalscreen)
//    SDL_LockSurface(physicalscreen);
//
//
//  SSetPixel(physicalscreen, ((int)(MAPWIDTH/2+(x1)*220/(1000-(y1))/DIV)), ((int)(MAPHEIGHT/3+MAPWIDTH*220/(1000-(y1)))), textcolor);
//
//  if SDL_MUSTLOCK(physicalscreen)
//    SDL_UnlockSurface(physicalscreen);
//}
...
//DKS
static inline void vga_drawpixel(int x1, int y1) {
	// We do it this way so we can have clipping
  SDL_Rect tmp_rect;

  tmp_rect.x = ((int)(MAPWIDTH/2+(x1)*220/(1000-(y1))/DIV));
  tmp_rect.y = ((int)(MAPHEIGHT/3+MAPWIDTH*220/(1000-(y1))));
  tmp_rect.w = 1;
  tmp_rect.h = 1;

#ifdef GCW
  SDL_FillRect(physicalscreen, &tmp_rect, colors_RGBA[textcolor]);
#else 
  SDL_FillRect(physicalscreen, &tmp_rect, colors_16bit[textcolor]);
#endif
}
